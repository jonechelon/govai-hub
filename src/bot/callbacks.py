    ...
    try:
        ...
        if data == "start":
            ...
            try:
                ...
                pass
        elif query.data == "gov:stake":
            # Show loading state immediately
            try:
                await query.edit_message_text(
                    "<i>⏳ Fetching your CELO balance...</i>",
                    parse_mode=ParseMode.HTML,
                )
            except BadRequest:
                pass
            ...
            # Fetch native CELO balance via RPC
            celo_balance = 0.0
            native_rpc_failed = False
            if wallet:
                try:

                    def _get_native_balance() -> float:
                        w3 = Web3(
                            Web3.HTTPProvider(
                    ...
                    celo_balance = await asyncio.wait_for(
                        loop.run_in_executor(None, _get_native_balance),
                        timeout=8.0,
                    )
                except Exception as e:
                    logger.warning("[RPC_ERROR] on-chain call failed: %s", e)
                    native_rpc_failed = True

            # Fetch stCELO balance (ERC-20) + CELO/stCELO exchange rate (Manager.toCelo on-chain)
            stcelo_data = await get_stcelo_data(wallet_address=wallet)
            rate = stcelo_data.get("exchange_rate")
            if isinstance(rate, (int, float)) and rate is not None:
                rate_fmt = f"{float(rate):.6f}".rstrip("0").rstrip(".")
                rate_line = (
                    f"1 <b>stCELO</b> ≈ <b>{hesc(rate_fmt)} CELO</b>."
                )
            else:
                rate_line = "<i>Exchange rate unavailable right now.</i>"

            if wallet and native_rpc_failed:
                try:
                    await query.edit_message_text(
                    ...
                    if not await _notify_html_parse_error(
                        query, query.data or "", e
                    ):
                        pass
                return
            ...
            if wallet:
                body = (
                    f"<b>💧 Liquid Staking</b>\n\n"
                    f"You have <b>{celo_balance:.4f} CELO</b> idle.\n"
                    f"{rate_line}\n\n"
                    f"<i>Wallet: <code>{hesc(truncate_wallet(wallet))}</code></i>"
                )
            else:
                body = (
                    "<b>💧 Liquid Staking</b>\n\n"
            ...
            convert_keyboard = (
                InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🔄 Convert to stCELO", callback_data="stake:convert"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "⬅️ Back",
                                callback_data="governance_menu",
                            )
                        ],
                    ]
                )
                if wallet
                else get_governance_keyboard(back_callback="governance_menu")
            )
            ...
            try:
                ...
                if not await _notify_html_parse_error(query, query.data or "", e):
                    pass
        elif query.data == "stake:convert":
            convert_body = (
                "<b>🔄 Convert CELO to stCELO</b>\n\n"
                "Swaps are signed <b>in your wallet</b> (Valora, MiniPay, MetaMask, etc.). "
                "This bot never holds your keys or broadcasts transactions.\n\n"
                "Use the links below to verify the token contract and open a CELO → stCELO swap."
            )
            convert_keyboard = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "📋 stCELO token (Celoscan)",
                            url=STCELO_TOKEN_CELOSCAN_URL,
                        ),
                        InlineKeyboardButton(
                            "Valora",
                    ...
                    [
                        InlineKeyboardButton(
                            "Ubeswap (CELO → stCELO)",
                            url=UBESWAP_CELO_TO_STCELO_URL,
                        )
                    ],
                    # Back row — separate from url buttons per §4 of ui_protection.mdc
                    [InlineKeyboardButton("⬅️ Back", callback_data="gov:stake")],
                ]
            )
            try:
                await query.edit_message_text(
