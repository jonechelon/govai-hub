...
def get_digest_keyboard(digest_id: str) -> InlineKeyboardMarkup:
    """Build the action keyboard shown below a delivered digest.

    Args:
        digest_id: unique identifier for the digest (e.g. ISO date string).

    Returns:
        InlineKeyboardMarkup with Details, Links, Ask AI, and Settings buttons.
    """
...
def get_premium_keyboard() -> InlineKeyboardMarkup:
    """Build the premium subscription options keyboard.

    Returns:
        InlineKeyboardMarkup with weekly, monthly, and confirm buttons.
    """
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⭐ 7 days — 0.50 cUSD", callback_data="premium:weekly"
                ),
                InlineKeyboardButton(
                    "⭐ 30 days — 1.50 cUSD", callback_data="premium:monthly"
                ),
            ],
            [
                InlineKeyboardButton(
                    "✅ I sent — /confirm_payment", callback_data="premium:confirm"
                )
            ],
        ]
    )
...
def get_settings_keyboard(subscribed: bool) -> InlineKeyboardMarkup:
    """Build the settings keyboard.

    Args:
        subscribed: current subscription status of the user.

    Returns:
        InlineKeyboardMarkup with subscribe/unsubscribe toggle.
    """
    toggle_label = "🔕 Unsubscribe" if subscribed else "🔔 Subscribe"
    toggle_data = "settings:unsubscribe" if subscribed else "settings:subscribe"
    return InlineKeyboardMarkup(
        [
